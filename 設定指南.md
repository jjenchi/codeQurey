...
> 若你是從 Charles 的原始 repo fork 而來，`charles` remote 僅供參考，請勿 push 到該 repo。
...
**有網域（建議，含 HTTPS）：** 完成 `部署清單.md` 3.10 節後：

- 查詢頁面：`https://你的網域/index.html`
- 管理頁面：`https://你的網域/admin.html`
- 大陸寬頻備用：`http://你的網域:8090/index.html`
...
查詢頁右上角 **「清除對話」** 可清空對話紀錄，下一個提問不會帶入之前的上下文。
